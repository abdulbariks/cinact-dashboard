    ...
    formatter: (value: string) => (
      <span
        className={`capitalize px-2.5 py-1 rounded-full text-sm font-medium ${
          teacherStatusColors[value?.toLowerCase?.() || value] ||
          "bg-[#443c29] text-[#ECAD11]"
        }`}
      >
        {value || "-"}
      </span>
  ...
  },
  {
    ...
    formatter: (_: any, row: any) => (
      <div className="flex items-center gap-4">
        {/* <Link
          href={`/dashboard/teacher-management/edit-teacher/${row.id}`}
          className="hover:bg-[#282e44] p-1.5 rounded-lg cursor-pointer"
        > */}
        <Link
          href={`/dashboard/user-management/edit-teacher/${row.id}`}
          className="hover:bg-[#282e44] p-1.5 rounded-lg cursor-pointer"
        >
          <EditIcon />
        </Link>
        ...
        <Link
          href={`/dashboard/user-management/teacher-details/${row.id}`}
          className="hover:bg-[#282e44] p-1.5 rounded-lg cursor-pointer"
        >
          <EyeIcon />
        </Link>
