...
function ChangeAttendenceSelect({ initialStatus }: { initialStatus: string }) {
	...
	return (
		<Select value={status} onValueChange={(value) => setStatus(value as ChangeAttendanceStatus)}>
			<SelectTrigger className={`h-8 min-w-30 border-[#3D4566] capitalize [&>svg]:hidden ${triggerColorClass}`}>
			...
			<SelectContent className="border-none bg-[#082420] text-[#777980]">
				<SelectItem value="present" className="capitalize text-[#18CC3F] data-highlighted:text-[#18CC3F]">
					present
				</SelectItem>
				<SelectItem value="absent" className="capitalize text-[#E9201D] data-highlighted:text-[#E9201D]">
					absent
				</SelectItem>
			</SelectContent>
		</Select>
...
export const attendenceListColumns: AttendanceColumn[] = [
	{
		label: "Student Name",
	...
	{
		label: "Attendance By",
		accessor: "attendence_by",
		width: "140px",
		formatter: (value: string) => (
			<span className="capitalize text-sm text-white font-medium">{value}</span>
		),
	},
	{
		...
		accessor: "status",
		width: "170px",
		formatter: (value: string) => <ChangeAttendenceSelect initialStatus={value} />,
	},
];
