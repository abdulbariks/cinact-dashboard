...
export default function StudentDetails() {
  return (
    <div>
      ...
      <h2 className=' text-2xl text-[#E6E7E8] font-semibold mt-8'>Student Details</h2>
        ...
        <div className=' bg-[#07121D] p-4  rounded-[10px] mt-5'>
          <div className=' flex flex-col lg:flex-row items-center  justify-between'>
            <div className=' flex items-center gap-3'>
              ...
              <div>
                <h3 className=' text-white text-lg font-medium'>Sophie Lambert <span className=' text-sm text-[#A5A5AB]'>(6y exp)</span> </h3>
          ...
          <div className=' mt-4'>
            ...
            <p className=' text-sm text-[#DFE1E7] mt-1.5'>Aspiring actor passionate about stage, screen,
              and voice performance.Currently training at
              CINACT to grow my performance skills and
              ...
              paymentHistoryData.map((item,index)=>(
                <div key={ index} className=' bg-[#07121D] p-4 rounded-[10px]'>
                  <div className=' flex items-center gap-2.5'>
                    <h3 className=' text-base text-white font-medium'>{item.name}</h3>
                    <p className={`${item.status==='paid'?' text-[#18CC3F] bg-[#2a3d2e] ':'text-[#FFC943] bg-[#423c2f]'} py-1 px-2.5 rounded-full text-sm `}>{item.status}</p>
                  </div>

                  <div className=' flex items-center justify-between mt-3'>
                    <p className=' text-sm text-[#A5A5AB]'>Data: {item.date}</p>
                    <p className=' text-base text-[#18CC3F] font-medium'>${item.amount}</p>
                  </div>
