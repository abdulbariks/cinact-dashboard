            ...
            <div key={index} className=' p-4 bg-[#07121d] rounded-[10px] '>
              <div className=' flex items-center justify-between'>
                ...
                <div className=' flex items-center gap-1'>
                  <p className=' text-xs text-[#18CC3F] bg-[#1a2538] py-1 px-1.5 inline-block rounded-full'>{item.students} students</p>
                  <div className=' bg-[#1a2538] inline-flex items-center gap-1 py-1 px-1.5 rounded-full'>
                    <p className={` text-xs  ${item.parcentage < '50' ? 'text-[#E9201D]' : 'text-white'}`}>{item.parcentage}</p>
                    {
