               ...
               <div className=' space-y-3 mt-4'>
                ...
                }
                {
                  items.map((item,index)=>(
                    <div key={index} className=' bg-[#07121d] p-4 rounded-[10px]'>
                      <h2 className=' text-white text-base font-medium'>{item.classTitle || item.title || item.module || '-'}</h2>
                      <p className=' text-sm text-[#D2D2D5] mt-1'>{item.course || item.inst_name || item.instructorName || '-'}</p>
                      ...
                      <div className=' flex items-center gap-3 mt-3'>
                        ...
                        <div className=' flex items-center gap-1'>
                          <ClockIcon/>
                          <p className=' text-white text-sm'>{formatTime(item.startTime || item.time, item.date)}</p>
                        </div>
                      </div>
