...
export default function ChatArea({ chatId }: ChatAreaProps) {
  ...
  // // Identify "Me" and Fetch Messages
  ...
  //     // Get Profile Info to identify current user
  //     const profile = await ChatsService.getMe({ token });
  //     setCurrentUserId(profile?.data?.data?.id || null);

  //     // Get Conversations
  //     const res = await ChatsService.getConversationById({ id: chatId, token });
  //     setMessages(res?.data?.items || []);
  //   } catch (error: any) {
  //     showErrorToast(error?.message || "Failed to load chat");
  //   } finally {
  //     setIsLoading(false);
  ...
  //   //       // if (msg.kind === "TEXT" && !msg.content?.text) return prev;
  //   //       return [...prev, msg];
  //   //     });
  //   //   }
  //   // });
